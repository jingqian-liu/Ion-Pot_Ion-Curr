...
# Set RESUME_RUN_DIR to resume from an existing run, e.g.:
#   RESUME_RUN_DIR=/data/.../outputs/unet_full193_20260410_005732 VARIANTS=add GPU_ID=6 bash train_unet_full_grid.sh
...
if [[ -n "${RESUME_RUN_DIR}" ]]; then
  OUT_ROOT="${RESUME_RUN_DIR}"
  RUN_SET_ID="$(basename "${OUT_ROOT}")"
  echo "[INFO] Resuming from existing run: ${OUT_ROOT}"
else
...
run_variant() {
  ...
  # Resume from checkpoint if RESUME_RUN_DIR is set
  if [[ -n "${RESUME_RUN_DIR}" ]]; then
    local latest_ckpt
    latest_ckpt=$(ls -t "${run_dir}/train/model-"*.pt 2>/dev/null | head -1)
    if [[ -n "${latest_ckpt}" ]]; then
      cmd+=(--resume-trainer --trainer-ckpt-path "${latest_ckpt}")
      echo "[INFO] Resuming variant '${run_name}' from checkpoint: ${latest_ckpt}"
    else
      echo "[WARN] No checkpoint found for '${run_name}' in ${run_dir}/train/ — starting fresh"
    fi
  fi
