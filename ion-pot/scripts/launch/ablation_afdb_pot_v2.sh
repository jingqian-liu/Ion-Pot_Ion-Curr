  ...
  IONIC_CONDITIONING="${IONIC_CONDITIONING:-add}"   # add | film
  ...
  # Ablation selection:
  #   ABLATIONS=all
  #   ABLATIONS=baseline,steps12k,batch4,steps12k_batch4
  ...
  if [[ "${IONIC_CONDITIONING}" != "add" && "${IONIC_CONDITIONING}" != "film" ]]; then
    echo "[ERR] IONIC_CONDITIONING must be one of: add, film"
    echo "      Got: ${IONIC_CONDITIONING}"
    exit 1
  fi
...

  {
    ...
    echo "SAVE_AND_EVAL_EVERY=${SAVE_AND_EVAL_EVERY}"
    ...
    echo "IONIC_CONDITIONING=${IONIC_CONDITIONING}"
  ...
  run_one() {
    ...
    if [[ "${MAPPER_VERSION}" == "fno" ]]; then
      cmd+=(--trainer.no-do-bf16-training)
    fi
  ...
  # Recommended comparable ablation suite
  # 1) baseline
  # 2) longer training
  # 3) larger micro-batch
  # 4) longer training + larger micro-batch
  # run_one baseline
  # run_one steps12k --trainer.train-num-steps 12000
  # run_one batch4 --trainer.train-batch-size 4
  run_one steps12k_batch4 --trainer.train-num-steps 12000 --trainer.train-batch-size 4
  run_one steps12k_batch8 --trainer.train-num-steps 12000 --trainer.train-batch-size 8
  run_one steps24k_batch4 --trainer.train-num-steps 24000 --trainer.train-batch-size 4
  run_one steps24k_batch8 --trainer.train-num-steps 24000 --trainer.train-batch-size 8

  # Optional exploratory runs (uncomment if needed):
  # run_one lr7e4 --trainer.train-lr 7e-4
  # run_one crop10 --trainer.random-crop-atom-num 10
  #
  # Non-comparable metric run (changes eval mask):
